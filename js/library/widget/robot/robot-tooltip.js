...
//define("jquery/tooltip/1.0.0/tooltip-debug", ["jquery/jquery/1.10.2/jquery-debug", "jquery/tooltip/1.0.0/tooltip-debug.css"], function (require) {
//  require('jquery/tooltip/1.0.0/tooltip-debug.css');
define('library/widget/robot/robot-tooltip',
    ...
    Tooltip.prototype = {
      ...
      render: function (data) {
        ...
        if (typeof(data) == "object") {
	      	html += '<dl  class="area-chart-tip">';
	      	if(data.name){
	      		html += '<dt class="title">' + data.name + '</dt>';
	      	}
	      	html += '<dd class="item">';
	      	if(data.tipsName){
		      	html += '<label class="name">' + data.tipsName + '：</label>';
	      	}
	      	if(data.value){
		      	html += '<em class="value">' + data.value + '</em>';
	      	}
	      	html += '</dd>';
		      html += '<dd class="item">';
		      if(data.rateName){
		      	html += '<label class="name">' + data.rateName + '：</label>';
		      }
		      if(data.rate){
		      	html += '<em class="value">' + data.rate +'</em>';
	      	}
		      html += '</dd>';
	      	html += '</dl>';
        }
        this.content = html;
        $('.tooltip-content').empty().append(html);
        this.sc = {      //重新计算dom大小
